...
2번째 인수(fromIndex)가 전달되면 검색 시작 위치를 fromIndex로 이동하여 역방향으로 검색을 시작한다. 이때 검색 범위는 0~fromIndex이며 반환값은 indexOf 메소드와 동일하게 발견된 곳의 index이다. 
...
## 3.5 String.prototype.replace(searchValue: string | RegExp, replaceValue: string | replacer: (substring: string, ...args: any[]) => string): string
...
## 3.9 String.rptotype.toLowerCase(): string
...
# Reference
[poiemaweb.com/js-string](https://poiemaweb.com/js-string)
